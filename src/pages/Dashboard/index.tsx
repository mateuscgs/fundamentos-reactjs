...
const formatTransactions = (transactions: Transaction[]): Transaction[] => {
  return transactions.map(transaction => ({
    ...transaction,
    formattedValue: `${signal[transaction.type]}\
      ${formatValue(transaction.value)}`,
    formattedDate: formatDate(transaction.created_at),
  }));
};

const formatBalance = (balance: ApiBalance): Balance => {
  return {
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      ...
      const { transactions: ts, balance: b } = response.data;
      setTransactions(formatTransactions(ts));
